...
class QuanLySinhVien:
    ...
    def nhap_sinh_vien(self):
        while True:
            ...
            if loai_sinh_vien == "0":
                print("kết thúc nhap thong tin sinh vien")
                break
            ...
            if loai_sinh_vien == "it":
                ...
                sv = svpoly.SinhVienIT(ten_sinh_vien,loai_sinh_vien, java, html, css)
                self.danh_sach.append(sv)
            elif loai_sinh_vien == "biz":
                marketing = float(input("Điểm Marketing: "))
                sales = float(input("Điểm Sales: "))
                sv = svpoly.SinhVienBiz(ten_sinh_vien,loai_sinh_vien, marketing, sales)
                self.danh_sach.append(sv)
            else:
